...
class Tasks extends Security_Controller {
    ...
    public function timer($task_id, $action = "start") {
    $this->load->model("Task_timer_model");
    $user_id = $this->session->userdata("user_id");

    if ($action == "start") {
        $this->Task_timer_model->start_timer($task_id, $user_id);
    } else {
        $this->Task_timer_model->stop_timer($task_id, $user_id);
    }
